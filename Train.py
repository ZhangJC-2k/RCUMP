...
if __name__ == "__main__":
    ...
    for epoch in range(300):
        ...
        for step, (g, label, Phi_batch, Phi_s_batch) in enumerate(loader_train):
            ...
            psnr = compare_psnr(label.detach().cpu().numpy(), out[opt.layernum-1].detach().cpu().numpy(), data_range=1.0)
            psnr_total = psnr_total + psnr

            loss = loss_f(mse, out[opt.layernum-1], label) + 0.7 * loss_f(mse, out[opt.layernum-2], label) + \
                   0.5 * loss_f(mse, out[opt.layernum-3], label) + 0.3 * loss_f(mse, out[opt.layernum-4], label)
